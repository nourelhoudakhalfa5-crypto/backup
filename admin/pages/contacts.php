...
if (isset($_GET['action']) && $_GET['action'] === 'delete' && isset($_GET['id'])) {
    ...
    $stmt->execute();
    redirectTo('contacts.php?deleted=1');
}
...
// Get contacts
$contacts = [];
$result = $conn->query("SELECT * FROM contacts ORDER BY date_envoi DESC LIMIT $offset, $perPage");
if ($result) {
...
// Stats
$unreadCount = 0;
$statsResult = $conn->query("SELECT COUNT(*) as total FROM contacts");
$totalMessages = $statsResult ? $statsResult->fetch_assoc()['total'] : 0;
?>
<!DOCTYPE html>
<html lang="fr">
<head>
...
<body class="bg-bg-dark text-text-primary font-inter min-h-screen">
    <div class="flex h-screen overflow-hidden">
        ...
        <div class="flex-1 flex flex-col overflow-hidden lg:ml-0">
            <header class="flex items-center justify-between h-16 px-4 lg:px-6 glass border-b border-white/10">
                ...
                <form method="POST" class="flex items-center gap-2" onsubmit="return confirm('Êtes-vous sûr de vouloir supprimer les messages sélectionnés?')">
            ...
            <main class="flex-1 overflow-y-auto p-4 lg:p-6">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6">
                    <div class="admin-card p-5">
                        ...
                        <div class="font-orbitron text-2xl font-bold"><?= number_format($totalMessages) ?></div>
                        <div class="text-sm text-text-secondary">Total Messages</div>
                    </div>
                </div>
                            ...
                            <table class="w-full admin-table">
                                ...
                                <tbody>
                                    <?php if (empty($contacts)): ?>
                                        <tr>
                                            <td colspan="8" class="px-4 py-8 text-center text-text-secondary">Aucun message trouvé</td>
                                        </tr>
                                    <?php else: ?>
                                        <?php foreach ($contacts as $contact): ?>
                                            <tr class="contact-row" data-name="<?= strtolower($contact['nom']) ?>" data-email="<?= strtolower($contact['email']) ?>">
                                                <td class="px-4 py-3">
                                                    <input type="checkbox" name="selected_ids[]" value="<?= $contact['id'] ?>" class="contact-checkbox rounded bg-white/10 border-white/20">
                                                </td>
                                                <td class="px-4 py-3"><?= $contact['id'] ?></td>
                                                <td class="px-4 py-3 font-medium"><?= escape($contact['nom']) ?></td>
                                                <td class="px-4 py-3"><?= escape($contact['email']) ?></td>
                                                <td class="px-4 py-3"><?= escape($contact['sujet'] ?? '-') ?></td>
                                                <td class="px-4 py-3 max-w-xs truncate"><?= escape($contact['message']) ?></td>
                                                <td class="px-4 py-3"><?= date('d/m/Y H:i', strtotime($contact['date_envoi'])) ?></td>
                                                <td class="px-4 py-3">
                                                    <div class="flex items-center justify-end gap-2">
                                                        <button type="button" onclick="viewMessage(<?= $contact['id'] ?>, '<?= escape(addslashes($contact['nom'])) ?>', '<?= escape(addslashes($contact['email'])) ?>', '<?= escape(addslashes($contact['sujet'] ?? '')) ?>', '<?= escape(addslashes($contact['message'])) ?>', '<?= date('d/m/Y H:i', strtotime($contact['date_envoi'])) ?>')" 
    ...
    <div id="message-modal" class="fixed inset-0 z-50 hidden">
        ...
        <div class="flex items-center justify-center min-h-screen p-4">
            <div class="relative bg-[#0a0a0a] border border-white/10 rounded-2xl p-6 w-full max-w-lg shadow-2xl">
                ...
                <h3 class="font-orbitron text-xl text-primary mb-4">Message</h3>
                <div class="space-y-4">
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            <div class="text-sm text-text-secondary">Nom</div>
                            <div class="font-medium" id="modal-nom"></div>
                        </div>
                        <div>
                            <div class="text-sm text-text-secondary">Email</div>
                            <div class="font-medium" id="modal-email"></div>
                        </div>
                    </div>
                    <div>
                        <div class="text-sm text-text-secondary">Sujet</div>
                        <div class="font-medium" id="modal-sujet"></div>
                    </div>
                    <div>
                        <div class="text-sm text-text-secondary">Message</div>
                        <div class="bg-white/5 rounded-lg p-4 mt-2 text-sm" id="modal-message"></div>
                    </div>
    ...
    <script>
        ...
        // Modal functions
        function viewMessage(id, nom, email, sujet, message, date) {
            ...
            document.getElementById('modal-date').textContent = 'Envoyé le: ' + date;
